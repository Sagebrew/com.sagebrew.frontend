...
def deployment(stalk, branch_key):
    ...
    bean_bucket = "massive-dangerzone-%s" % environ.get("CIRCLE_BRANCH", "")

    sys_util_name = "mass-sys"
...
def create_app_version_update_env(branch_key, stalk, sha_key, bean_bucket,
    ...
    aws_env = "/home/ubuntu/%s/aws_bundle/%s-%s.zip" % ("com.sagebrew.frontend",
                                                        sha_key, branch_key)
    option_tuple = populate_options(aws_env)
...
def staging_deploy():
    stalk = layer1.Layer1(
        aws_access_key_id=environ.get("AWS_ACCESS_KEY_STAGING", ""),
...
def production_deploy():
    stalk = layer1.Layer1(
        aws_access_key_id=environ.get("AWS_ACCESS_KEY_PROD", ""),
